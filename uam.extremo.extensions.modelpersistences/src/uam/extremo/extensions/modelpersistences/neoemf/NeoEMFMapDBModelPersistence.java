...
public class NeoEMFMapDBModelPersistence extends ExtensibleModelPersistenceImpl {
	...
	public NeoEMFMapDBModelPersistence() {
		...
		sharedEditingDomain.getResourceSet().getResourceFactoryRegistry().getProtocolToFactoryMap().put(MapDbURI.SCHEME, PersistentResourceFactory.getInstance());
	}
	...
	public boolean load(String path) {
		if(resourceDb == null){
			try {
				...
				resourceDb = sharedEditingDomain.getResourceSet().createResource(MapDbURI.createFileURI(file));
				return true;
			} 
			catch (Exception e) {
				return false;
