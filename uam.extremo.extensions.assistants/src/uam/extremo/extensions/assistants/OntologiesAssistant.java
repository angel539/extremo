...
public class OntologiesAssistant extends ExtensibleFormatAssistantImpl {
		...
		if(ontology != null){
			for(OWLClass owlClass : ontology.getClassesInSignature(true)){
				...
				for (OWLNamedIndividual ind : individuals) {
					...
					guardSemanticNode(semanticResource, individualSemanticNode);
						...
						if(!domain.isAnonymous()){
							if(domain.asOWLClass().getIRI().toString().compareTo(iri.toString()) == 0){
								...
								for(IRI rangeURI : rangeURIs){
									SemanticNode range = searchSemanticNodeById(semanticResource, rangeURI);
			...
			if(ontology.containsClassInSignature(iriClass, true)){
				...
				for(OWLClassExpression superclass : superclasses){
					if(! superclass.isAnonymous()){
						SemanticNode range = searchSemanticNodeById(semanticResource, superclass.asOWLClass().getIRI());
			...
			if(ontology.containsObjectPropertyInSignature(iriClass, true)){
				...
				for(OWLDataPropertyExpression superproperty : superproperties){
					if(! superproperty.isAnonymous()){
						Property namedId = searchPropertyById(semanticResource, superproperty.asOWLDataProperty().getIRI());
		...
		if(ontology != null && parent.getTrace() != null && parent.getTrace() instanceof IRI){
			...
			if(ontology.containsObjectPropertyInSignature(iriClass, true)){
				...
				for(OWLObjectPropertyExpression superproperty : superproperties){
					if(! superproperty.isAnonymous()){
						Property namedId = searchPropertyById(semanticResource, superproperty.asOWLObjectProperty().getIRI());
	...
	public void toInverseOf(ObjectProperty parent) {
		if(ontology != null && parent.getTrace() != null && parent.getTrace() instanceof IRI){
			...
			if(ontology.containsObjectPropertyInSignature(iriClass, true)){
				...
				if(inverseOf != null){
					Property namedId = searchPropertyById(semanticResource, inverseOf.asOWLObjectProperty().getIRI());
