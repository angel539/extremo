...
public class RelationshipGraphViewPart extends ViewPart implements IZoomableWorkbenchPart{
	public static final String ID = "uam.extremo.ui.zest.views.RelationGraphViewPart";
	...
	public RelationshipGraphViewPart() {
	}
					...
					Job job = new Job("Graph"){
						...
						protected IStatus run(IProgressMonitor monitor) {					
							...
							try {
									...
									for(IFormatAssistant assistant : AssistantFactory.getInstance().getAssistans()){
										if(resource.getAssistant().compareTo(((FormatAssistant)assistant).getNameExtension()) == 0){
											...
											for(SemanticNode classInResource : classesInResource){
												List<ObjectProperty> references = new ArrayList<ObjectProperty>();
								...
								for(Resource resource : resources) input.addAll(resource.getClasses());
 	...
 	private void syncWithUI(List<SemanticNode> input) {
 		Display.getDefault().asyncExec(new Runnable() {
 			public void run() {
 				...
 				MessageDialog.openInformation(getSite().getShell(), "Draw Graph Relationship Dependencies", "Dependencies calculated");
 			}
 		});
 	}
	...
	public void createPartControl(Composite parent) {
		...
		viewer.setContentProvider(new RelationshipViewPartNodeContentProvider());
	    viewer.setLabelProvider(new RelationshipGraphViewPartLabelProvider());
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				RelationshipGraphViewPart.this.fillContextMenu(manager);
			}
		});
