package automaton.internal.dk.brics.automaton;
...
final public class StringUnionOperations {
	 ...
	 */
	private static automaton.internal.dk.brics.automaton.State convert(State s, 
			IdentityHashMap<State, automaton.internal.dk.brics.automaton.State> visited) {
		automaton.internal.dk.brics.automaton.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new automaton.internal.dk.brics.automaton.State();
	 ...
	 */
	public static automaton.internal.dk.brics.automaton.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations(); 

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, automaton.internal.dk.brics.automaton.State>());
	}
