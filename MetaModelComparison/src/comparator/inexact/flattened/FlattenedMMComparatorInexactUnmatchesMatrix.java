...
public class FlattenedMMComparatorInexactUnmatchesMatrix extends MMComparator{
	...
	public FlattenedMMComparatorInexactUnmatchesMatrix() throws IOException {
		super();
	}
	...
	protected void generateStatsMatches() throws IOException{
		...
		for(Entry<Tuple<File, File>, List<ENamedElement>> entry : this.unmatchesLeft.entrySet()){
			...
			long countLeft = eObjects.stream().filter(e -> e instanceof EClass).count();
		...
		for(Entry<Tuple<File, File>, List<ENamedElement>> entry : this.unmatchesRight.entrySet()){
			...
			long countRight = eObjects.stream().filter(e -> e instanceof EClass).count();
        ...
        FileOutputStream fileOut = new FileOutputStream("logs/inexact-flattened/stats/inexact-flattened-unmatches-matrix.xlsx");
	...
	protected void generateFilesMatches() throws IOException {
		...
		for(Entry<Tuple<File, File>, List<ENamedElement>> entry : this.unmatchesLeft.entrySet()){
			...
			String fileNameX = FilenameUtils.removeExtension(tuple.x.getName());
			String fileNameY = FilenameUtils.removeExtension(tuple.y.getName());
			Path file = Paths.get("logs/inexact-flattened/unmatches-matrix/" + fileNameX + "-" + fileNameY + "-left" + ".txt");
		...
		for(Entry<Tuple<File, File>, List<ENamedElement>> entry : this.unmatchesRight.entrySet()){
			...
			String fileNameX = FilenameUtils.removeExtension(tuple.x.getName());
			String fileNameY = FilenameUtils.removeExtension(tuple.y.getName());
			Path file = Paths.get("logs/inexact-flattened/unmatches-matrix/" + fileNameX + "-" + fileNameY + "-right" + ".txt");
