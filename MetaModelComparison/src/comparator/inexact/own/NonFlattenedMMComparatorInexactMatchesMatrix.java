...
public class NonFlattenedMMComparatorInexactMatchesMatrix extends MMComparator{
	public NonFlattenedMMComparatorInexactMatchesMatrix() throws IOException {
		super();
	}
	...
	protected void generateStatsMatches() throws IOException{
        ...
        FileOutputStream fileOut = new FileOutputStream("logs/inexact-nonflattened/stats/inexact-matches-matrix.xlsx");
	...
	protected void generateFilesMatches() throws IOException {
		...
		for(Entry<Tuple<File, File>, Map<ENamedElement, ENamedElement>> entry : this.matchesPerModel.entrySet()){
			...
			String fileNameX = FilenameUtils.removeExtension(tuple.x.getName());
			String fileNameY = FilenameUtils.removeExtension(tuple.y.getName());
			Path file = Paths.get("logs/inexact-nonflattened/matches-matrix/" + fileNameX + "-" + fileNameY + ".txt");
